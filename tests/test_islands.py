...
def test_seed_all_islands_inserts_starters_into_every_island(db):
    mgr = IslandsManager(db, num_islands=4)
    mgr.seed_all_islands(STARTERS)
    for island_id in range(4):
        rows = db.top_k_in_island(island_id=island_id, k=10)
        assert {r.architecture_spec["name"] for r in rows} == {
            arch.name for arch, _ in STARTERS
        }


def test_pick_island_returns_valid_id(db):
    mgr = IslandsManager(db, num_islands=8, rng=random.Random(0))
    for _ in range(50):
        assert 0 <= mgr.pick_island() < 8


def test_rank_by_mean_fitness_orders_strong_islands_first(db):
    mgr = IslandsManager(db, num_islands=4)
    db.insert(
    ...
    db.insert(low_arch, low_scores, island_id=0)
    ranked = mgr.rank_by_mean_fitness()
    assert ranked[0] == 2
    assert ranked.index(0) > ranked.index(2)


def test_maybe_reset_returns_none_outside_cadence(db):
    mgr = IslandsManager(db, num_islands=4, reset_every_generations=10)
    mgr.seed_all_islands(STARTERS)
    assert mgr.maybe_reset(current_generation=5) is None
    assert mgr.maybe_reset(current_generation=0) is None


def test_maybe_reset_wipes_weak_islands_at_cadence(db):
    mgr = IslandsManager(
        db, num_islands=4, reset_every_generations=10, top_seed_count=2
    )
    ...
    for island_id in (2, 3):
        db.insert(low_arch, low_scores, island_id=island_id)
    ...
    for weak_island in (2, 3):
        alive = db.top_k_in_island(island_id=weak_island, k=10)
        assert {r.architecture_spec["name"] for r in alive} == {"Satoshi"}


def test_maybe_reset_no_op_when_no_seeds_available(db):
    mgr = IslandsManager(db, num_islands=4, reset_every_generations=10)
    assert mgr.maybe_reset(current_generation=10) is None


def test_diversity_summary_returns_one_entry_per_island(db):
    mgr = IslandsManager(db, num_islands=4)
    mgr.seed_all_islands(STARTERS)
...
def test_constructor_rejects_too_few_islands(db):
    with pytest.raises(ValueError):
        IslandsManager(db, num_islands=1)
