...
class IslandsManager:
    ...
    def seed_all_islands(self, starters: list[tuple[Architecture, Scores]]) -> None:
        ...
        for island_id in range(self.num_islands):
            for architecture, scores in starters:
                self.db.insert(
                    architecture, scores, island_id=island_id, generation=0
                )
    ...
    def rank_by_mean_fitness(self) -> list[int]:
        """Island ids sorted by mean alive fitness, highest first."""
        means = self.db.mean_fitness_per_island(self.num_islands)
    ...
    def maybe_reset(self, current_generation: int) -> ResetEvent | None:
        ...
        seeds = self.db.top_programs_from_islands(strong, self.top_seed_count)
        seed_ids = [s.id for s in seeds]
        if not seed_ids:
            return None

        for island_id in weak:
            self.db.reset_island(island_id, seed_ids)
    ...
    def diversity_summary(self) -> dict[int, float]:
        """Per-island diversity metric, keyed by island id."""
        return {i: self.db.diversity_metric(i) for i in range(self.num_islands)}
